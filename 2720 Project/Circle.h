...
private:
	...
	int64_t circleSize; // of the drawn block in pixels

	int64_t red, green, blue; // rbg color ratio of the circle

	int64_t thickness; // of the square

public:
	// constructor
	Circle_t(const Display_t &d, const int64_t &fps, const shared_ptr<BackGround_t> &groundPtrInput) {
		...
		// set random size
		circleSize = (rand() % kDiameterVariance) + kMinDiameter;
		...
		thickness = (rand() % kCircleThicknessVariance) + kMinCircleThickness;
	...
	// draws the circle based on its attributes
	void draw() {
		al_draw_ellipse(currentPosition.x, currentPosition.y - kCircleGroundOffset - circleSize, circleSize, circleSize, al_map_rgb(red, green, blue), thickness);
	}
